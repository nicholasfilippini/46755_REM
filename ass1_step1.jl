...
if termination_status(Step1) == MOI.OPTIMAL
    ...
    @printf "The total production from conventional generators is: %0.3f\n" sum(generator_values)

    wind_values = .0[]
    for w=1:W
        w_value = value.(pw[w])
        println("The production of w$(w) = $(w_value)")
        append!(wind_values, w_value)
    end
    @printf "The total production from wind farms is: %0.3f\n" sum(wind_values)
